...
function addInPlace(leftMatrix, rightMatrix) {
    if (leftMatrix.rows != rightMatrix.rows || leftMatrix.cols != rightMatrix.cols)
        throw new Error("Dimesion does not match for operation:add");
...
function minusInPlace(leftMatrix, rightMatrix) {
    if (leftMatrix.rows != rightMatrix.rows || leftMatrix.cols != rightMatrix.cols)
        throw new Error("Dimesion does not match for operation:minus");
    var rows = leftMatrix.rows, cols = leftMatrix.cols;
    for (var i = 0; i < rows; i++) {
        for (var j = 0; j < cols; j++) {
            leftMatrix.val[i][j] -= rightMatrix.val[i][j];
...
function mul(leftMat, rightMat) {
    if (leftMat.cols != rightMat.rows)
        throw new Error("Dimesion does not match for operation:muitiply");
    ...
    for (var i = 0; i < m; i++) {
        for (var j = 0; j < p; j++) {
            ...
            for (var it = 0; it < n; it++)
                val += leftMat.val[i][it] * rightMat.val[it][j];
...
function dotMulInPlace(leftMat, rightMat) {
    if (leftMat.rows != rightMat.rows || leftMat.cols != rightMat.cols)
        throw new Error("Dimesion does not match for operation:dot muitiply");
...
function mul_gpu(leftMat, rightMat) {
    ...
    var mulfunction_string = mulfunction_part_1 + n.toString() + mulfunction_part_2;
...
function x_mul_xT(inMat) {
    ...
    var mulfunction_string = mulfunction_part_1 + x_col.toString() + mulfunction_part_2;
