...
class mat {
    ...
    dimensions() { return Array.from([this.rows, this.cols]); }
    isVector() { return this.rows <= 1; }
    ...
    copy(inputMat) { this.init(inputMat.val); this.cols = inputMat.cols; this.rows = inputMat.rows; return this; }
    equals(inMat, EPSILON = 0.0001) {
    ...
    zeros(row, col) { return this.Ns(row, col, 0); }
    ones(row, col) { return this.Ns(row, col, 1); }
    ...
    adds(val) { this.each(function (eachMatrixValue) { return eachMatrixValue + val; }); return this; }
    muls(val) { this.each(function (eachMatrixValue) { return eachMatrixValue * val; }); return this; }
    minuss(val) { return this.adds(val * (-1)); }
    divs(val) { return this.muls(1.0 / val); }
    //matrix operations. All matrix operation member functions are NOT In Place
    add(rightMat) { return m.add(this, rightMat); }
    minus(rightMat) { return m.minus(this, rightMat); }
    mul(rightMat) {
    ...
    mul_gpu(rightMat) { var result = m.mul_gpu(this, rightMat); this.copy(result); return this; }
    ...
    [Symbol.for('==')](rightOperand, EPSILON = 0.0001) {
        //if right operand is a raw array of number or 2D array, initialize the matrix first
        if (Array.isArray(rightOperand)) {
            var rightOperandMatrix = new mat(rightOperand);
    ...
    //setter and getter
    set(row, col, val) { this.dimCheck(row, col); this.val[row][col] = val; return this; }
    get(row, col) { this.dimCheck(row, col); return this.val[row][col]; }
    at(row, col) { this.dimCheck(row, col); return this.val[row][col]; }
    //get a row vector (as an N-by-1 matrix) by index
    row(rowIndex) { this.dimCheck(rowIndex, 0); return new mat().initVec(this.val[rowIndex]); }
    ...
    //return a 2D array
    to2DArray() { return this.clone().val; }
    //return a 1D array
    toArray() { return [].concat(...this.val); }
    //reshape matrix
    reshape(row, col) {
    ...
    toString() { return m.mat2csv(this); }
    //output the whole information to console
    log() { console.log(this); return this; } //output in console
}
export { mat };
